...
const initOrUpdateContacts = async (accounts) => {
  ...
  console.log(
    'The first time it sould be {} and the second it should cointain addresses'
  )
  console.log('existingDoc now its: ', existingDoc.content)

  if (existingDoc.length > 0) {
    //There are already docs on the DB
    const newAddressess = await updateAccountsDocument(existingDoc, accounts)
    const poapLink = ''
    return { newAddressess, poapLink }
  } else {
    //to check that your stream was created you can paste its stream id from
    //console here
    const streamId =
      'kjzl6cwe1jw1479xb0qdpfljg6nzx3tc5lvkt8az0mktdx9cpb8al21y83dxmar'
...
const updateAccountsDocument = async (existingDoc, accounts) => {
  ...
  if (newAddressess.length > 0) {
    ...
    //we can return the new addresses, so on the UI it'd say smth like
    //you have this new interactions: Would you like to save them?
