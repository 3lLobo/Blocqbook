...
export function CoinBalance({ item, quote_currency }) {
  ...
  const balanceInEth = parseFloat(balance / Math.pow(10, contract_decimals)).toPrecision(3)
    ...
    <div className="grid grid-cols-5 justify-center place-items-center even:bg-indigo-100 rounded-bl-xl rounded-tr-xl px-3">
      ...
      <div className="col-span-2">
        <div className=" grid grid-cols-2 space-x-1">
          <div className="text-sm text-indigo-900  text-right">{balanceInEth}</div>
          <div className="text-sm font-semibold text-left">{contract_ticker_symbol}</div>
        </div>
      </div>
      <div className="col-span-2">
        <div className=" grid grid-cols-2 space-x-1 justify-center">
          <div className="text-sm text-indigo-900 text-right "> {quoteInUsd}</div>
          <div className="text-sm font-semibold text-left">{quote_currency}</div>
        </div>
      </div>
    </div>
  )
