...
export function getRandomTailwindColor() {
  const color = tailwindColors.colors[Math.floor(Math.random() * tailwindColors.colors.length)]
  const shade = tailwindColors.shades[Math.floor(Math.random() * tailwindColors.shades.length)]
  return `${color}-${shade}`
}
